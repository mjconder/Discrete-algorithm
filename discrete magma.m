...
IsDense := function (A, B, p : precision:=20)
    ...
    if IsDiscrete(A,B,p) eq true then
      return false;
    elif {0} eq {TL(A,p), TL(B,p), TL(A*B,p)} then
      return false;
    elif Trace(A*B*A^-1*B^-1) eq 2 then
      return false;
    else
      return true;
    end if;
...
K2 := pAdicField(p2);
J := Matrix(2,[0,1,-1,0]);
I2 := Matrix(2,[Sqrt(K2!-1),0,0,-Sqrt(K2!-1)]);
I1 := Matrix(2,[Sqrt(K1!-2),1,1,-Sqrt(K1!-2)]);

X1 := Id + I1 - J + 3*I1*J;
Y1 := -9*Id -7*I1 -J + 7*I1*J;

X2 := Id + I2 - J + 3*I2*J;
Y2 := -9*Id -7*I2 -J + 7*I2*J;




// this generates "random" pairs of matrices in SL(2,Q_p)
// (although they all seem to be either not discrete or discrete and free)

RandomPair := function(p : precision:=100)
   K := pAdicField(p);
   O := RingOfIntegers(K);
   gens := [];
   for i in [1,2] do
      repeat a := Random(O)/Random(O);
      until a ne 0;
      b := Random(O)/Random(O);
      c := Random(O)/Random(O);
      d := (1 + b * c) / a;
...
p:=7; list:=[]; num_steps:=0;
for i in [1..10000] do
   X,Y := RandomPair(p);
   a,b := IsDiscrete(X,Y,p);
   num_steps+:=1;
   if a eq true and b ne ", G is discrete and free of rank two" then
      Append(~list,[X,Y]);
   end if;
end for;
#list;
